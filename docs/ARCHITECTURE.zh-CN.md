...
3. Profile transport：本地 CLI、浏览器自动化、桌面 adapter 等外部能力只返回受约束的 artifact。本仓库包含一个 GPT Pro browser/CDP reference adapter，但真实部署时的 transport 选择仍应放在私有 profile/contract 中。
...
不是所有 gate 都是 human-only。GPT Pro review 使用 orchestrator/owner-proxy authorization 路径。只有项目契约明确标记为 human-only 的 action，例如 master plan 修改或破坏性操作，才需要当前 human owner。
